...
import { TopBar, BottomBar, MainLayout, BookPane, AIPane, CollaborationPanel, VersionHistoryPanel, FocusModeControls, } from "./components";
import { useBookStore, useAIStore, useUIStore, useCollaborationStore } from "./state";
...
const BookApp = () => {
    ...
    const handleContentChangeWithVersion = useCallback((chapterId, content) => {
    ...
    const handleSendMessage = async (text) => {
        ...
        try {
            ...
            if (response) {
                try {
                    await handleSuggestionRequest(`Based on the latest assistant response, provide a staged Generation → Reflection → Critique → Editor workflow that improves the chapter. Original prompt (truncated): ${text
                        .slice(0, 160)
                        .replace(/\s+/g, " ")}.`, "rewrite", {
                        context: chapterContext,
    ...
    const handleSearchWeb = async (query) => {
        addMessage({
            id: `${Date.now()}`,
            role: "user",
    ...
    const handleSendCollabMessage = (text) => {
        addCollabMessage({
            id: `msg-${Date.now()}`,
    ...
    const handleJumpToCollaborator = (collaborator) => {
    ...
    return (_jsxs(ThemeProvider, { theme: theme === "dark" ? darkTheme : lightTheme, children: [_jsx(CssBaseline, {}), _jsxs(Box, { sx: { width: "100vw", height: "100vh", overflow: "hidden" }, children: [_jsx(Fade, { in: !focusModeActive, children: _jsx(Box, { children: _jsx(TopBar, { projectTitle: "My Book Draft", onSave: handleSave, onExport: handleExport }) }) }), _jsxs(Box, { sx: {
                            display: "flex",
                            width: "100%",
                        ...
                        }, children: [panelVisibility.collaboration && (_jsx(CollaborationPanel, { collaborators: collaborators, messages: collabMessages, currentUserId: currentUserId, sessionId: sessionId || undefined, onInvite: handleInviteCollaborator, onSendMessage: handleSendCollabMessage, onJumpToCollaborator: handleJumpToCollaborator, collapsed: !panelVisibility.collaboration, onToggleCollapse: () => togglePanel("collaboration") })), _jsx(Box, { sx: { flex: 1, minWidth: 0, display: "flex", overflow: "hidden" }, children: _jsx(MainLayout, { left: panelVisibility.chapterNav ? (_jsx(BookPane, { chapters: chapters, activeChapterId: activeChapterId, onSelectChapter: setActiveChapter, onContentChange: handleContentChangeWithVersion, onAddChapter: handleAddChapter, forceCollapseDrawer: focusModeActive, isFocusMode: focusModeActive })) : null, right: panelVisibility.aiPane ? (_jsx(AIPane, { messages: messages, suggestions: suggestions, loading: aiLoading, onSendMessage: handleSendMessage, onInsertSuggestion: handleInsertSuggestion, onDismissSuggestion: handleDismissSuggestion, onGenerateImage: handleGenerateImage, onSearchWeb: handleSearchWeb })) : null, defaultLeftWidth: panelVisibility.chapterNav && panelVisibility.aiPane ? 60 : 100 }) }), panelVisibility.versionHistory && (_jsx(VersionHistoryPanel, { versions: versions, onRestore: handleRestoreVersion, onCompare: handleCompareVersion, collapsed: !panelVisibility.versionHistory, onToggleCollapse: () => togglePanel("versionHistory") }))] }), _jsx(Fade, { in: !focusModeActive, children: _jsx(Box, { children: _jsx(BottomBar, { aiMode: aiMode, onAIModeChange: setAIMode, onExportPDF: handleExportPDF, onExportDOCX: handleExportDOCX, onCollaborate: handleCollaborate }) }) }), _jsx(FocusModeControls, { focusModeActive: focusModeActive, panelVisibility: panelVisibility, onToggleFocusMode: toggleFocusMode, onTogglePanel: togglePanel })] })] }));
};
