...
import { Collaborator, CollabMessage, VersionEntry, PanelVisibility } from "../components";
...
interface CollaborationState {
  ...
  lastPrimaryPanel: "chapterNav" | "aiPane";
...
const loadLastPrimaryPanel = (): "chapterNav" | "aiPane" => {
  if (typeof window !== "undefined") {
...
export const useCollaborationStore = create<CollaborationState>((set, get) => ({
  ...
  setFocusModeActive: (active) => {
    ...
    if (active) {
      ...
      const focusVisibility: PanelVisibility = {
        collaboration: false,
        versionHistory: false,
      ...
      if (typeof window !== "undefined") {
        ...
        if (saved) {
          try {
            restoredVisibility = JSON.parse(saved) as PanelVisibility;
          } catch {
